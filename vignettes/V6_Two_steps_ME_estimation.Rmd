...
with 1 reestimation the CV-error decreases and the stays stable.\
...
`coef = coef.dataThesis` of `plot` allows to obtain the trace 
 
```{r,echo=TRUE,eval=TRUE, fig.width=6,fig.height=4,fig.align='center'}
plot(res.lmgce.1se.twosteps, which = 7, coef = coef.dataThesis)$p7
```

We can see that the precision error remains stable.
