...
assume that $z_k^{upper}=100$, $k\in\left\lbrace 0,\dots,4\right\rbrace$ is a 
...
The estimated GME coefficients are $\widehat{\boldsymbol{\beta}}^{GME_{(100)}}=$ `r paste0("(", paste(round(coef(res.lmgce.100.GME), 3), collapse = ", "), ")")`. Note that now we have used the dual optimization problem (see ["Optimization"](V4_Optimization.html)).  

```{r,echo=TRUE,eval=TRUE}
(coef.res.lmgce.100.GME <- coef(res.lmgce.100.GME))
```

But if there is some information, for instance, on $\beta_1$, that
can be reflected on `support.signal.points`. Lets suppose that one suspects that
$\beta_1=0$. Since the support spaces are centered in zero one can assign
a higher probability to the support point in or around the center. One can set 
$\mathbf{q_1}=(0.1, 0.1, 0.6, 0.1, 0.1)'$, for instance. `support.signal.points` accepts information on the distribution of probabilities in the form of a $(K+1)\times M$ matrix. The first line corresponds to $\mathbf{q_0}$, the second to $\mathbf{q_1}$, and so on.
...
`r round(GCEstim::accmeasure(fitted(res.lmgce.100.GME), dataThesis$y, which = "RMSE"), 3)`
and $RMSE^{y,GCE_{{100}}} \approx$
`r round(GCEstim::accmeasure(fitted(res.lmgce.100.GCE), dataThesis$y, which = "RMSE"), 3)`) 
as well as the prediction cross-validation errors (
$CV\text{-}RMSE_5^{y,GME_{(100)}} \approx$
`r round(res.lmgce.100.GME$error.measure.cv.mean, 3)`
and $CV\text{-}RMSE_5^{y,GCE_{{100}}} \approx$
`r round(res.lmgce.100.GCE$error.measure.cv.mean, 3)`).  
The precision error is slightly lower for the GCE approach: $RMSE^{\beta,GME_{(100)}} \approx$
`r round(GCEstim::accmeasure(coef.res.lmgce.100.GME, coef.dataThesis, which = "RMSE"), 3)`
and $RMSE^{\beta,GCE_{(100)}} \approx$
`r round(GCEstim::accmeasure(coef.res.lmgce.100.GCE, coef.dataThesis, which = "RMSE"), 3)`.
