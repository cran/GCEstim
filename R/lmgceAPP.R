...
#'
#' @return \code{NULL}. This function is called for its side effect (launching the app).
#'
#' @export
lmgceAPP <- function() {
  appDir <- system.file("shiny-examples", "lmgceAPP", package = "GCEstim")
  if (appDir == "") {
    stop("Could not find example directory. Try re-installing `GCEstim`.", call. = FALSE)
  }
