  ...
  if (is.null(which)) {
    ...
    if (which == "mode") {
      if (!is.null(seed))
        set.seed(seed)
      if (OLS) {
        apply(object$results$bootstrap$coef.matrix.OLS, 1, function(x) {
      ...
      if (OLS) {
        apply(object$results$bootstrap$coef.matrix.OLS, 1, median)
      } else {
...
#' limits for each parameter. Generally, these will be labelled as (1-level)/2
                                 ...
                                 ...)
{
  ...
  if (which == "estimates") {
    if (OLS) {
      mcf <- object$results$bootstrap$coef.matrix.OLS
    } else {
...
plot.cv.tsbootgce <-
  function (x,
      ...
      for (i in 1:nrow(object$results$bootstrap$coef.matrix)) {
        ...
        for (k in 1:length(ci.levels)) {
          LL <-
            c(LL,
              as.numeric(
                apply(object$meboot[[i]],
                      1,
                      function(x){quantile(x,
                                           probs = (1 - ci.levels[k])/2)})))
          UL <-
            c(UL,
              as.numeric(
                apply(
                  object$meboot[[i]],
                  1,
                  function(x){quantile(x, probs = (1 + ci.levels[k])/2)})))
          Median <- c(Median, as.numeric(apply(object$meboot[[i]], 1, median)))
        }

        plot_data <-
          data.frame(
            Time = rep(as.numeric(time(object$data.ts[, 1])),
                       length(ci.levels)),
      ...
      for (i in 1:nrow(object$results$bootstrap$coef.matrix)) {
        ...
        p2 <-
          ...
          ggplot2::ggtitle(
            latex2exp::TeX(
              paste0(
                {if(ci.method == "hdr") "HDR of " else "CI of "},
                "$b_",
    ...
    if (show[4L]) {
      ...
      for (i in 1:nrow(object$results$bootstrap$coef.matrix.OLS)) {
        x <- as.numeric(object$results$bootstrap$coef.matrix.OLS[i, ])
        h = hdrcde::hdrbw(hdrcde::BoxCox(x, lambda), mean(prob))
        if (lambda == 1) {
          den <- density(x, bw = h, n = 1001)
        } else {
          ...
          ggplot2::ggtitle(
            latex2exp::TeX(
              paste0(
                ...
                "$b_",
                i-ifelse("(Intercept)" %in% rownames(
                  object$results$bootstrap$coef.matrix.OLS),
                  1,
                  0),
                "$",
                " estimates - ",
                rownames(object$results$bootstrap$coef.matrix.OLS)[i]))) +
          ggplot2::ylim(ylim) +
          ggplot2::theme_minimal() +
          ggplot2::theme(axis.line = ggplot2::element_line(color='black'),
                         plot.background = ggplot2::element_blank(),
