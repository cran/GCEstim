...
`dataThesis` (see ["Generalized Maximum Entropy
...
Assume that $z_k^{upper}=50$, $k\in\left\lbrace 0,\dots,4\right\rbrace$
...
outputting $1$. Note that `dual.BFGS` converged while `dual.CG` did not. Note that the default optimization setting were kept. Conjugate gradient methods will generally be more fragile but may be successful in much larger optimization problems.  
