...
\begin{align}
    \qquad \qquad \qquad \qquad \qquad \mathbf{y} = \mathbf{X}\boldsymbol{\beta} + \mathbf{e}, \qquad \qquad \qquad \qquad \qquad (1)
\end{align} 
...
matrix and $\mathbf{e}$ is a $(N \times 1)$ vector of random
...
$E\left[ \mathbf{e} | \mathbf{X}\right]=\mathbf{0}$ and
$E[\mathbf{ee}'|\mathbf{X}]=\sigma^2\mathbf{I}$,
...
$e_n, \space n\in\left\lbrace 1,\dots,N\right\rbrace$, as a
...
To define an ill-conditioned design matrix of predictors (remove the column of ones corresponding to the intercept), $\mathbf{X}_{pred}$, with a
specific condition number value, namely $cn=200$, the traditional
singular value decomposition will be obtained and the singular values in
$\mathbf{S}$, a diagonal matrix with the same dimension of $\mathbf{X}_{pred}$,
will be modified such that
$cond(\mathbf{X}_{pred})=cond(\mathbf{USV}')=cn$, where $\mathbf{U}$ and
$\mathbf{V}$ are square unitary matrices, and $cond(\mathbf{X_{pred}'X_{pred}})=cn^2$.
...
```{recho=TRUE,eval=FALSE}
coef.dataThesis <- c(-4, 0, -16, -12, -5)

dataThesis <- 
  fngendata(
    n = 75,
    cont.k = 1,
    y.gen.cont.beta = coef.dataThesis[3:5],
    intercept.beta = coef.dataThesis[1],
...
The following matrix shows the distribution of variables generated and the Pearson coefficients of correlation between them.  
...
$k\in\left\lbrace 0,\dots,4\right\rbrace$ is a "wide upper bound" for
...
The estimated OLS coefficients are $\widehat{\boldsymbol{\beta}}^{OLS}=$ `r paste0("(", paste(round(coef(lm(y ~ ., data = dataThesis)), 3), collapse = ", "), ")")`.  
...
Consider the root mean squared error as a measure of fit of prediction of an estimator $\mathcal{E}$ ($RMSE^{y,\mathcal{E}}$) defined as

\begin{equation}
  RMSE^{y,\mathcal{E}} = \sqrt{\frac{\sum_{n=1}^N {\left( y_n - \hat y_n^\mathcal{E} \right)^2}}{N}},
\end{equation} where $\hat y_n^\mathcal{E}=X_{n,.}~\boldsymbol{\hat\beta^\mathcal{E}}$.

The prediction errors are approximately equal:
$RMSE^{y,GME_{(100000)}} \approx$
`r round(GCEstim::accmeasure(fitted(res.lmgce.100000), dataThesis$y, which = "RMSE"), 3)`
and $RMSE_{\mathbf{\hat y}}^{OLS} \approx$
`r round(GCEstim::accmeasure(fitted(res.lm), dataThesis$y, which = "RMSE"), 3)`.
...
\begin{equation}
  CV\text{-}RMSE_\kappa^{y,\mathcal{E}}=\frac{\sum_{cv=1}^\kappa RMSE^{y_{cv},\mathcal{E}}}{\kappa},
\end{equation} 
where $\hat y_{n,(cv)}^\mathcal{E}=X_{(cv)_{n,.}}\boldsymbol{\hat\beta}_{(cv)}^\mathcal{E}$ and
$\boldsymbol{\hat\beta}_{(cv)}^\mathcal{E}$ was estimated in 
\begin{eqnarray}
...
$CV\text{-}RMSE_5^{y,GME_{(100000)}} \approx$
`r round(res.lmgce.100000$error.measure.cv.mean, 3)`
and $CV\text{-}RMSE_5^{y,OLS} \approx$
...
precision.  
Let $RMSE^{\beta,\mathcal{E}}$ be

\begin{equation}
  RMSE^{\beta,\mathcal{E}} = \sqrt{\frac{\sum_{k=0}^K {\left( \beta_k - \hat\beta_k^\mathcal{E} \right)^2}}{K+1}}.
\end{equation}

The precision errors are similar in magnitude with a slight advantage for the 
GME approach: $RMSE^{\beta, GME_{(100000)}} \approx$
`r round(GCEstim::accmeasure(coef.res.lmgce.100000, coef.dataThesis, which = "RMSE"), 3)`
and $RMSE^{\beta,OLS} \approx$
`r round(GCEstim::accmeasure(coef.res.lm, coef.dataThesis, which = "RMSE"), 3)`.
...
$k\in\left\lbrace 0,\dots,4\right\rbrace$ was a "wide upper bound" for
...
$RMSE^{y,GME_{(100)}} \approx$
`r round(GCEstim::accmeasure(fitted(res.lmgce.100), dataThesis$y, which = "RMSE"), 3)` 
and $CV\text{-}RMSE_\kappa^{y,GME_{(100)}} \approx$
`r round(res.lmgce.100$error.measure.cv.mean, 3)`,
but a significant decrease was found for the precision error,
$RMSE^{\beta,GME_{(100)}} \approx$
`r round(GCEstim::accmeasure(coef.res.lmgce.100, coef.dataThesis, which = "RMSE"), 3)`.
...
If $z_k^{upper}=50$ then $\widehat{\beta}^{GME_{(50)}}=$ `r paste0("(", paste(round(coef(res.lmgce.50), 3), collapse = ", "), ")")`, $RMSE^{y,GME_{(50)}} \approx$
`r round(GCEstim::accmeasure(fitted(res.lmgce.50), dataThesis$y, which = "RMSE"), 3)`, $CV\text{-}RMSE_5^{y,GME_{(50)}} \approx$
`r round(res.lmgce.50$error.measure.cv.mean, 3)` and $RMSE^{\beta,GME_{(50)}} \approx$
`r round(GCEstim::accmeasure(coef.res.lmgce.50, coef.dataThesis, which = "RMSE"), 3)`.  
...
$\mathbf{z}_0'= \left[ -12, -6, 0, 6, 12\right]$,
$\mathbf{z}_1' = \left[ -5, -2.5, 0, 2.5, 5\right]$,
$\mathbf{z}_2'= \left[ -30, -15, 0, 15, 30\right]$,
$\mathbf{z}_3'= \left[ -26, -13, 0, 13, 26\right]$, and
$\mathbf{z}_4'= \left[ -16, -8, 0, 8, 16\right]$.
...
$RMSE^{y,GME_{(apriori.centered.zero)}} \approx$
`r round(GCEstim::accmeasure(fitted(res.lmgce.apriori.centered.zero), dataThesis$y, which = "RMSE"), 3)`, as well as the cross-validation prediction error, $CV\text{-}RMSE_5^{y,GME_{(apriori.centered.zero)}} \approx$
...
$RMSE^{\beta,GME_{(apriori.centered.zero)}} \approx$
`r round(GCEstim::accmeasure(coef.lmgce.apriori.centered.zero, coef.dataThesis, which = "RMSE"), 3)`.  
...
$\mathbf{z}_0'= \left[ -6, -5, -4, -3, -2\right]$,
$\mathbf{z}_1' = \left[ -2, -1, 0, 1, 2\right]$,
$\mathbf{z}_2'= \left[ -18, -17, -16, -15, -14\right]$,
$\mathbf{z}_3'= \left[ -14, -13, -12, -11, -10\right]$, and
$\mathbf{z}_4'= \left[ -7, -6, -5, -4, -3\right]$.
...
The prediction error ($RMSE^{y,GME_{(apriori.centered.beta)}} \approx$
`r round(GCEstim::accmeasure(fitted(res.lmgce.apriori.centered.beta), dataThesis$y, which = "RMSE"), 3)`), the cross-validation prediction error ($CV\text{-}RMSE_5^{y,GME_{(apriori.centered.beta)}} \approx$
`r round(res.lmgce.apriori.centered.beta$error.measure.cv.mean, 3)`) and precision error ($RMSE^{\beta,GME_{(apriori.centered.beta)}} \approx$
`r round(GCEstim::accmeasure(coef.lmgce.apriori.centered.beta, coef.dataThesis, which = "RMSE"), 3)`) improved when compared with the centered at zero approach.
